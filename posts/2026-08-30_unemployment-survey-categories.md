...
- **[All of it is pullable from one API](#getting-the-data-bls-api-series-ids)** — series IDs for all six rates, plus the v1/v2 limits, and the one thing the BLS API cannot give you: revision history [[14]](#source-14)
...
Before the programs, the categories. Every box below comes from one survey, the CPS, and the whole structure starts from the civilian noninstitutional population, age 16 and over, which excludes active-duty military and people in institutions such as prisons and nursing homes [[11]](#source-11). Which survey produces what comes next.
...
## Getting the Data: BLS API Series IDs

Everything above is pullable programmatically. The obstacle is not access, it's that
the API has no series catalog — you have to already know the series ID you want [[14]](#source-14).

### The endpoints

Six of them, all under `https://api.bls.gov/publicAPI/v2/`. Version 1 exposes the
same paths under `/publicAPI/v1/` without a key, at lower limits [[15]](#source-15).

| Method | Path | Returns |
|---|---|---|
| GET | `/timeseries/data/{seriesID}` | One series, most recent three years |
| GET | `/timeseries/data/{seriesID}?latest=true` | Only the latest observation |
| POST | `/timeseries/data/` | 1 to 50 series, with year range and options |
| GET | `/timeseries/popular` | The 25 most-requested series overall |
| GET | `/timeseries/popular?survey={abbr}` | The 25 most-requested within one survey |
| GET | `/surveys` | Every BLS survey with its two-letter abbreviation |
| GET | `/surveys/{abbr}` | Metadata for a single survey |

The GET form is the quick one but it silently caps at three years, so anything
historical has to go through POST. Register for a key at
[data.bls.gov/registrationEngine](https://data.bls.gov/registrationEngine/) [[14]](#source-14).

### The POST body

```json
{
  "seriesid": ["LNS14000000", "LNS13327709"],
  "startyear": "2006",
  "endyear": "2026",
  "catalog": true,
  "calculations": true,
  "annualaverage": true,
  "registrationkey": "your-32-character-key"
}
```

`calculations` returns net and percent change over 1, 3, 6, and 12 months.
`catalog` returns survey, area, item, and seasonality metadata, but only for
surveys covered by the BLS Data Finder. `annualaverage` adds the `M13` period.
...
The 20-year cap is a hard truncation, not an error — ask for 1948 to 2026 and you
get silently trimmed. Long histories need chunked requests stitched together.

### The response envelope

Every call returns the same shape: `status`, `responseTime`, `message[]`, and
`Results.series[]`. Each series carries `seriesID` and a `data[]` array in
**reverse chronological order**, each point keyed by `year` and `period` —
`M01` through `M12` monthly, `M13` annual average, `Q01`–`Q04` quarterly,
`A01` annual [[15]](#source-15).

### The six rates

All seasonally adjusted, monthly, from the CPS [[16]](#source-16).

| Rate | BLS series ID | FRED alias |
|---|---|---|
| U-1 | `LNS13025670` | `U1RATE` |
| U-2 | `LNS14023621` | `U2RATE` |
| U-3 | `LNS14000000` | `UNRATE` |
| U-4 | `LNS13327707` | `U4RATE` |
| U-5 | `LNS13327708` | `U5RATE` |
| U-6 | `LNS13327709` | `U6RATE` |

Note U-2 breaks the pattern: it sits in the `LNS14` prefix while U-1, U-4, U-5,
and U-6 are all `LNS13`. Hardcoding the prefix is a mistake worth avoiding.

### Duration shares, the U-1 building blocks

| Series | BLS series ID |
|---|---|
| Unemployed less than 5 weeks (% of total) | `LNS13008397` |
| Unemployed 5 to 14 weeks | `LNS13025701` |
| Unemployed 15 to 26 weeks | `LNS13025702` |
| Unemployed 27 weeks and over | `LNS13025703` |

### Reading the series ID

CPS IDs decompose as `LN` + `S` or `U` + a six-digit measure code. `LNS` is
seasonally adjusted, `LNU` is not. Swapping that one character gives you the
unadjusted twin of any series above, which matters because the marginally
attached and discouraged levels are published unadjusted only.

### The catch for revision tracking

The BLS API returns current values only. There is no vintage parameter and no way
to ask what a series said last month. If you want revision history you need ALFRED,
which models vintages with `realtime_start` and `realtime_end`, or the CES vintage
data files. This is the single biggest constraint on building anything that studies
how these numbers change after publication.
...
<a id="source-1">**1.**</a> [U.S. Bureau of Labor Statistics — Employment Situation Technical Note](https://www.bls.gov/news.release/empsit.tn.htm) — CPS sample of about 60,000 eligible households; CES sample of about 119,000 businesses representing roughly 622,000 worksites; A tables vs. B tables; annual benchmark to unemployment insurance administrative records

<a id="source-2">**2.**</a> [U.S. Bureau of Labor Statistics — Current Population Survey](https://www.bls.gov/cps/) — Program home page; scope of published labor force, employment, and unemployment measures

<a id="source-3">**3.**</a> [U.S. Bureau of Labor Statistics — Current Employment Statistics (National)](https://www.bls.gov/ces/) — Nonfarm payroll employment, average weekly hours, and average hourly earnings by industry

<a id="source-4">**4.**</a> [U.S. Bureau of Labor Statistics — CES State and Area Questions and Answers](https://www.bls.gov/sae/questions-and-answers.htm) — 18-month benchmark replacement period running April of the prior year through September of the benchmark year; benchmarks applied to not seasonally adjusted estimates

<a id="source-5">**5.**</a> [U.S. Bureau of Labor Statistics — Quarterly Census of Employment and Wages](https://www.bls.gov/cew/) — Administrative census of UI-covered employers; county and detailed industry employment and wage counts

<a id="source-6">**6.**</a> [U.S. Bureau of Labor Statistics — Job Openings and Labor Turnover Survey](https://www.bls.gov/jlt/) — Job openings, hires, quits, layoffs and discharges, and other separations

<a id="source-7">**7.**</a> [U.S. Bureau of Labor Statistics — JOLTS Technical Note](https://www.bls.gov/news.release/jolts.tn.htm) — JOLTS weighted employment benchmarked to equal CES employment; birth/death model built from QCEW activity and projected forward using CES

<a id="source-8">**8.**</a> [U.S. Bureau of Labor Statistics — Local Area Unemployment Statistics](https://www.bls.gov/lau/) — Model-based state, metro, and county unemployment rates; annual re-estimation of historical series

<a id="source-9">**9.**</a> [U.S. Department of Labor, ETA — Unemployment Insurance Weekly Claims Data](https://oui.doleta.gov/unemploy/claims.asp) — Weekly release schedule, Thursday 8:30 a.m. Eastern; state-level claims data

<a id="source-10">**10.**</a> [U.S. Department of Labor — Unemployment Insurance Weekly Claims News Release](https://www.dol.gov/ui/data.pdf) — ETA 538 advance report counts claims by liable state; ETA 539 revision the following week counts claimants by state of residence; prior week routinely revised upward

<a id="source-11">**11.**</a> [U.S. Bureau of Labor Statistics — Concepts and Definitions (CPS)](https://www.bls.gov/cps/definitions.htm) — Definitions of unemployed, marginally attached, discouraged, part time for economic reasons; U-1 through U-6 formulas and denominators

<a id="source-12">**12.**</a> [U.S. Bureau of Labor Statistics — Employment Situation Release Archive](https://www.bls.gov/news.release/empsit.toc.htm) — Table A-15 alternative measures (seasonally adjusted); table A-16 persons not in the labor force (not seasonally adjusted)

<a id="source-13">**13.**</a> [U.S. Bureau of Labor Statistics — Monthly Employment Situation Report: Quick Guide to Methods and Measurement Issues](https://www.bls.gov/bls/empsitquickguide.htm) — Sampling error at 1.6 standard errors for a 90 percent interval; benchmark re-anchoring to QCEW March counts; five years of seasonally adjusted data recalculated with each benchmark

<a id="source-14">**14.**</a> [U.S. Bureau of Labor Statistics — Public Data API FAQs](https://www.bls.gov/developers/api_faqs.htm) — v1 vs v2 limits (500 vs 25 daily queries, 50 vs 25 series, 20 vs 10 years, 50 requests per 10 seconds); registration process; no series ID catalog

<a id="source-15">**15.**</a> [U.S. Bureau of Labor Statistics — API Signature v2](https://www.bls.gov/developers/api_signature_v2.htm) — POST signature, request parameters, and the year/period response envelope

<a id="source-16">**16.**</a> [FRED — Total Unemployed Plus Discouraged Workers (U-4)](https://fred.stlouisfed.org/series/U4RATE) — BLS source codes for the alternative measures, cross-referenced against the FRED aliases
